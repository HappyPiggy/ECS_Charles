...
/// 随机类型怪物移动系统
/// </summary>
public class RandomEnemyMoveSystem : IExecuteSystem, IInitializeSystem
{
    ...
    public RandomEnemyMoveSystem(Contexts contexts, Services services)
    {
    ...
    public void Execute()
    {
        ...
        if (contexts.game.gameProgress.state == GameProgressState.InGame)
        {
            ...
            {
                if (enemy.enemyType.value == EnemyType.Random)
                {
                    if (enemy.isMover)
                    {
